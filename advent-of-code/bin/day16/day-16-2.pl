...
sub query {
  my $aunt = shift;
  my $params = shift;
  my @order = sort { $a <=> $b } keys %$aunt;

  my @matching;
  foreach my $sue ( @order ) {
    my $as = $aunt->{$sue};
    next if defined $as->{children}    && $as->{children}    != $params->{children};
    next if defined $as->{cats}        && $as->{cats}        <= $params->{cats};
    next if defined $as->{samoyeds}    && $as->{samoyeds}    != $params->{samoyeds};
    next if defined $as->{pomeranians} && $as->{pomeranians} >= $params->{pomeranians};
    next if defined $as->{akitas}      && $as->{akitas}      != $params->{akitas};
    next if defined $as->{vizslas}     && $as->{vizslas}     != $params->{vizslas};
    next if defined $as->{goldfish}    && $as->{goldfish}    >= $params->{goldfish};
    next if defined $as->{trees}       && $as->{trees}       <= $params->{trees};
    next if defined $as->{cars}        && $as->{cars}        != $params->{cars};
    next if defined $as->{perfumes}    && $as->{perfumes}    != $params->{perfumes};
    push @matching, $sue;
  }

  if ( @matching == 0 ) {
    die "Unable to find a matching Aunt Sue\n";
  }
  if ( @matching > 1 ) {
    die "Found more than one matching Aunt Sue\n";
  }
  return $matching[0];
}
my $s = input();
...
printf "The gift was given by Sue # %d\n", query( $s, $q );
