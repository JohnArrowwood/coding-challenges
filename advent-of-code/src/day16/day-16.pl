...
EOF
  ...
  foreach ( split "\n", $text ) {
    ...
    $properties->{$2}{$3}{$1} = undef;
    $properties->{$4}{$5}{$1} = undef;
    $properties->{$6}{$7}{$1} = undef;
  }
  return $properties;
}
...
sub query {
  ...
  foreach my $property ( keys %$params ) {
    foreach my $sue ( keys %{$properties->{$property}{$params->{$property}}} ) {
      $scores->{$sue}++;
    }
  }
  my @order = sort { $scores->{$b} <=> $scores->{$a} } keys %$scores;

  my $sue = shift @order;
  my $second = shift @order;
  die "Not solvable" unless $scores->{$sue} > $scores->{$second};
  return $sue;
}
my $p = input();
my $q = {
