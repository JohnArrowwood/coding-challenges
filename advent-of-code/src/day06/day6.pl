...
# TODO: Make this more functional and less scripty
sub on { 
  $lights->[$_[0]][$_[1]] = 1;
}
sub off {
...
sub execute {
  my $s = shift;
  my ( $command, $x1, $y1, $x2, $y2 ) = (
    $s =~ m/^(toggle|turn (?:on|off)) (\d+),(\d+) through (\d+),(\d+)/ );
  my $f = ( $command eq 'toggle' ? \&toggle : $command eq 'turn on' ? \&on : \&off );
  my ( $x, $y );
  if ( $x1 > $x2 ) {
    $x = $x1;
    $x1 = $x2;
